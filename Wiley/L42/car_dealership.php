...
if($isFound){
	...
	for($i = 0; $i < $resultCount; $i++){
		...
		$results[$i] = new Object("model", $makes[strtolower($makesIndexes[$makeRand])][$modelRand]);// aici mai trebe completat
	}
}

$response = new Object("searchedCar", new Object("model", $model, "make", $make, "year", $year),
	"isFound", $isFound,
	"results", $results
	);
